...
namespace MobileApp.Libraries
{
    public class tttGame
    {
        ...
        private void Add(ref Grid grid, int row, int column)
        {
            ...
            tapped.Tapped += (sender, e) =>
            {
                if (!_won)
                {
                    ...
                    {
                        _won = true;
                        Show($"{_piece} wygrywa!", app_title);
                    }
                    else if (Drawn())
                    {
                        Show("Remis!", app_title);
                    }
                    else
                    {
                        _piece = (_piece == cross ? box : cross); // Swap Players
                    }
                }
                else
                {
                    Show("Koniec gry!", app_title);
                }
            };
        ...
        public async void New(ContentPage page, Grid grid)
        {
            ...
            Layout(ref grid);
            _won = false;
            _piece = await ConfirmAsync("Kto zaczyna?", app_title,
                box.ToString(), cross.ToString()) ? box : cross;
        }
    }
}
